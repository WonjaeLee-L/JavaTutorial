...
//			temp_carintime = in.nextLine();
	...
	public void delCar() {
		...
		if (caridx != -1) {
			...
			// 출차 추가요금 계산
			int addPay = carList.get(caridx).getCarpay()
					+ calPay(carList.get(caridx).getCarInTime(), carList.get(caridx).getCarOutTime());
	...
	public int calPayMinut(String inTime, String outTime) {
		...
		int amount = (int) Duration.between(parkedAt, exitAt).toMinutes(); // Duration, LocalTime을 통해 문자열로 주어진 시간의 차(분)를
																			// 구함
		return amount;
	} // 주차한 시간(분) 구하기
