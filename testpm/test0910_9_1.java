...
public class test0910_9_1 {
	...
	/* *** 문제점 ***
	 중복을 발견한 for문에서, 위에 값 배정, 랜덤 배정 등 여러 처리를 다 하려고 하다보니, 조건에 따라 for문이 길어지기도 하였고,
	 i, j의 변수 활용이 고정적으로 되어 다양하게 생각하지 못하였음. 3개 중복, 4개 중복을 나누어 반복문을 짰는데
	 반복되는 것을 편하게 하려고 for문을 이용했으나, 사실상 너무 복잡하게 이용함
	 for문에서 index값으로 i, j를 사용하는 것에 집중하다보니, for문 밖에서도 숫자로 그냥 표현하면 되는 것을 거의 배제하여
	 문제 푸는데에 스스로 제약을 걸었음. 이중 for문을 열어서 변경, 출력 등의 모든일을 그 안에서 전부 처리하려고 하여 다양하게 생각하지 못하였음
	 for문 하나에 한 역할을 하는게 좋다(최소한)
	*/
